...
- **Billing boundary.** Billing is on iff `DODO_PAYMENTS_API_KEY` is set (`isBillingEnabled`); off means
  unlimited free runs, so never make a code path require the payment provider. (Dodo Payments, not
  Stripe: Stripe onboarding is invite-only for Indian businesses.) A run's credit is spent in the
  ...
  granted by the signed Dodo webhook on `payment.succeeded`, keyed on the payment id
  (`credit_ledger.payment_ref`). `src/lib/dodo.ts` is app-only: the worker must not import it.
